...
class Day18Spec extends AnyFlatSpec with Matchers with Day18 {
  it should "solve part 1 test" in {
    ...
    p.parseAll(p.expr, repair("2 * 3 + (4 * 5)")).get shouldBe 26
    p.parseAll(p.expr, repair("5 + (8 * 3 + 9 + 3 * 4 * 3)")).get shouldBe 437
    p.parseAll(p.expr, repair("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))")).get shouldBe 12240
    p.parseAll(p.expr, repair("((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2")).get shouldBe 13632
  }

  def repair(str: String): String = str.replaceAll("\\(", " ( ").replaceAll("\\)", " ) ")

  it should "solve part 1" in {
    part1(Loader(this, "day18.txt").toIndexedSeq.map(repair)) shouldBe 31142189909908L
  }
  ...
  it should "solve part 2 test" in {
    ...
    p.parseAll(p.expr, "1 + ( 2 * 3 ) + ( 4 * ( 5 + 6 ) )").get shouldBe 51
    p.parseAll(p.expr, repair("2 * 3 + (4 * 5)")).get shouldBe 46
    p.parseAll(p.expr, repair("5 + (8 * 3 + 9 + 3 * 4 * 3)")).get shouldBe 1445
    p.parseAll(p.expr, repair("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))")).get shouldBe 669060
    p.parseAll(p.expr, repair("((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2")).get shouldBe 23340
  }
