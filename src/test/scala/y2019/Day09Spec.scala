...
class Day09Spec extends AnyFlatSpec with Matchers with Day09 {
  ...
  it should "solve part 1 tests" in {
    {
      ...
      machine.execute()
      machine.outputs.asScala.mkString(",") shouldBe code
    }
    ...
    }
    {
      ...
      val machine = Intcode.compiler(code).compile()
      machine.execute()
      machine.outputs.asScala.toSeq shouldBe Seq(1125899906842624L)
    }
  }
  ...
  it should "solve part 1" in {
    ...
    machine.execute()
    machine.outputs.asScala.toSeq shouldBe Seq(2662308295L)
  }

  it should "solve part 2" in {
    val machine = Intcode.compiler(input).compile(Seq(2L))
    machine.execute()
    machine.outputs.asScala.toSeq shouldBe Seq(63441L)
  }
}
