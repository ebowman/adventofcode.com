...
class Day03Spec extends AnyFlatSpec with Matchers with Day03 {
  ...
  it should "test part 2" in {
    val ones = solve2(testInput, '1')
    val zeros = solve2(testInput, '0')
    (toBinary(ones) * toBinary(zeros)) shouldBe 230
  }

  it should "solve part 2" in {
    val ones = solve2(inputs, '1')
    val zeros = solve2(inputs, '0')
    (toBinary(ones) * toBinary(zeros)) shouldBe 1353024
  }
}
