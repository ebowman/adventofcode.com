...
trait Day10 {
  ...
  def solve1(input: Seq[String]): Long = {
    ...
    @tailrec def recurse(stack: List[Char] = Nil)(i: String): Long =
      if i.isEmpty then 0
      else if opens.contains(i.head) then recurse(i.head :: stack)(i.tail)
      else if matcher(i.head) == stack.head then recurse(stack.tail)(i.tail)
      else scores(i.head)
  ...
  def solve2(input: Seq[String]): Long = {
    ...
    @tailrec def recurse(stack: List[Char] = Nil)(i: String): Option[String] =
      if i.isEmpty then Some(stack.mkString)
      else if opens.contains(i.head) then recurse(i.head :: stack)(i.tail)
      else if matcher(i.head) == stack.head then recurse(stack.tail)(i.tail)
      else None
