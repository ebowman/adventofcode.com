...
trait Day25 {
  def solve1(input: Seq[String]): Int = {
    ...
    val nextGen: Map[Char, ((Int, Int)) => (Int, Int)] = Map(
      '>' -> { pos: (Int, Int) => if (pos._1 == sizeX - 1) (0, pos._2) else (pos._1 + 1, pos._2) },
      'v' -> { pos: (Int, Int) => if (pos._2 == sizeY - 1) (pos._1, 0) else (pos._1, pos._2 + 1) })
    val initBoard = (for (y <- 0 until sizeY; x <- 0 until sizeX) yield {
      if (input(y)(x) != '.') Some((x, y) -> input(y)(x)) else None
    }).flatten.toMap

    def availableMoves(board: Map[(Int, Int), Char], c: Char): Seq[((Int, Int), Char)] =
      board.filterNot { case (pos, char) =>
        board.contains(nextGen(char)(pos)) || board.get(pos).contains(if (c == 'v') '>' else 'v')
      }.toSeq
    ...
    @tailrec def recurse(board: Map[(Int, Int), Char], n: Int = 0): Int = {
      ...
      if (done) (n + 1) / 2 + 1
      else if ((n % 2) == 0) recurse(advance(board, availableMoves(board, '>')), n + 1)
