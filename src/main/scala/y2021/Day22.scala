...
trait Day22 {
  def solve1(input: Seq[String]): Int = {
    ...
    input.foreach { line =>
      ...
      for x <- max(-50, xMin) to min(50, xMax)
           ...
           z <- max(-50, zMin) to min(50, zMax) do
        if line.startsWith("on") then core((x, y, z)) = true else core.remove((x, y, z))
    }
  ...
  case class Cube(x1: Int, x2: Int, y1: Int, y2: Int, z1: Int, z2: Int) {
    ...
    def nonEmpty: Option[Cube] = if x1 <= x2 && y1 <= y2 && z1 <= z2 then Some(this) else None
  ...
  object CubeFactory {
    ...
    def apply(line: String): Cube =
      summon[Mirror.Of[Cube]].fromProduct(listToTuple("""-?\d+""".r.findAllIn(line).map(_.toInt).toSeq).asInstanceOf[tuple])
  ...
  @tailrec final def solve2(input: Seq[String], cubes: Map[Cube, Int] = map()): Long = {
    if input.isEmpty then cubes.map { case (cube, sign) => cube.volume(sign) }.sum
    else {
