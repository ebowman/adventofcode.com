...
trait Day22 {
  ...
  def part1(input: Iterable[String]): Int = {
    ...
    def recurse(p1: Queue[Int], p2: Queue[Int]): Queue[Int] = {
      ...
      // else if (p2.isEmpty) p1
        ...
        if play1 > play2 then recurse(nextP1.enqueueAll(immutable.Iterable(play1, play2)), nextP2)
  ...
  def part2(input: Iterable[String]): Int = {
    def recursiveCombat(p1: Queue[Int], p2: Queue[Int], seen: Set[(Int, Int)] = Set()): (Int, Seq[Int]) = {
      if p2.isEmpty then (1, p1)
      else if p1.isEmpty then (2, p2)
      else {
        val hash = (p1.hashCode(), p2.hashCode())
        if seen.contains(hash) then (1, p1)
        else {
          val ((card1, p1next), (card2, p2next)) = (p1.dequeue, p2.dequeue)
          val winner =
            if p1next.size >= card1 && p2next.size >= card2 then recursiveCombat(p1next.take(card1), p2next.take(card2))._1
            else if card1 > card2 then 1 else 2
          if winner == 1 then recursiveCombat(p1next.enqueueAll[Int](immutable.Iterable(card1, card2)), p2next, seen + hash)
