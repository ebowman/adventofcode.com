...
trait Day20 {
  ...
  case class Tile(id: Int, tf: String = "", bits: Array[Array[Boolean]]) {
    ...
    def transform(f: (Int, Int) => Boolean, t: String): Tile = {
      ...
      for {row <- bits.indices; col <- bits.indices} {
    ...
    private def shift(b: Boolean, i: Int): Int = (if (b) 1 else 0) << i
  ...
  def parse(input: IndexedSeq[String]): IndexedSeq[Tile] = {
    ...
    def recurse(i: IndexedSeq[String], tiles: IndexedSeq[Tile] = IndexedSeq.empty): IndexedSeq[Tile] = {
      if (i.isEmpty) tiles
        ...
        case IdRe(id) =>
          recurse(i.drop(12),
            tiles :+ Tile(id.toInt, "", i.tail.take(10).map(line => line.map(c => if (c == '.') false else true)).map(_.toArray).toArray))
      }
    }
  ...
  final def recurseAcross(allTiles: IndexedSeq[Tile], accum: IndexedSeq[Row] = IndexedSeq.empty): IndexedSeq[Row] = {
    ...
    if (accum.nonEmpty && accum.head.tiles.size == goal) accum
  ...
  final def recurseDown(allRows: IndexedSeq[Row],
    ...
    if (accum.nonEmpty && accum.head.rows.length == allRows.head.tiles.size) accum
    else if (accum.isEmpty) {
