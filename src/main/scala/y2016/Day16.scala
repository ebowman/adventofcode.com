...
trait Day16 {
  ...
  @tailrec final def checksum(str: String): String = {
    ...
    for (i <- 0 until str.length - 1 by 2) if (str(i) == str(i + 1)) sb.append("1") else sb.append("0")
    if ((sb.length % 2) == 0) checksum(sb.toString()) else sb.toString()
  }

  @tailrec final def fill(str: String, bufSize: Int): String =
    if (str.length >= bufSize) str.take(bufSize) else fill(str + "0" + not(str.reverse), bufSize)
